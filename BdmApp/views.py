...
def enquiry_form(request):
    ...
    if request.method == "POST":
        ...
        # Optional: Auto-set source to 'Website' if you have that in DB
        # website_source = LeadSource.objects.get(name='Website')
        # lead.source = website_source
...
def register_student_from_lead(request, lead_id):
    ...
    if request.method == 'POST':
        ...
        # --- B. Payment Data ---
        try:
            pay_amount = float(request.POST.get('amount', 0)) # Down Payment
        except ValueError:
            pay_amount = 0.0
        ...
        try:
            installments_count = int(request.POST.get('installments', 1)) # Default 1
        except ValueError:
        ...
        if pay_mode == 'EMI':
            ...
            if balance > 0 and installments_count > 0:
                ...
                # Loop to create Installment Records
                for i in range(1, installments_count + 1):
                    # Calculate Due Date (Today + i months)
                    next_due_date = today + relativedelta(months=+i)
                    
                    FeeInstallment.objects.create(
                        student=student,
                        amount=round(emi_amount, 2), # Round to 2 decimals
                        due_date=next_due_date,
                        is_paid=False
                    )
            
        # 3. Check if Fully Paid (Buffer for float math)
        paid_so_far = student.feepayment_set.aggregate(total=Sum('amount'))['total'] or 0
...
def add_lead(request):
    ...
    File Location: templates/bdm/add_lead.html
    """
    courses = Course.objects.all()
    sources = LeadSource.objects.filter(is_active=True)

    if request.method == 'POST':
        # ... (Your existing BDM Add Lead logic here) ...
        # ... (first_name, age, qualification, status, etc.) ...
        
        messages.success(request, "Lead added successfully!")
        return redirect('lead_list')

    # IMPORTANT: Renders the BDM HTML file
    return render(request, 'bdm/add_lead.html', {'courses': courses, 'sources': sources})
login_required
@user_passes_test(is_bdm)
@transaction.atomic 
def register_student_from_lead(request, lead_id):
